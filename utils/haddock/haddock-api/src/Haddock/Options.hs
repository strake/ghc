...
modulePackageInfo :: PackageState
...
modulePackageInfo _pkgState _flags Nothing = (Nothing, Nothing)
modulePackageInfo pkgState flags (Just modu) =
  ( optPackageName flags    <|> fmap unitPackageName pkgDb
  , optPackageVersion flags <|> fmap unitPackageVersion pkgDb
  )
  where
    pkgDb = lookupUnit pkgState (moduleUnit modu)
