...
createInterface tm flags modMap instIfaceMap = do
  ...
  let ms             = pm_mod_summary . tm_parsed_module $ tm
      ...
      dflags@DynFlags { pkgState } = ms_hspp_opts ms
      !instances     = modInfoInstances mi
      !fam_instances = md_fam_insts md
      !exportedNames = modInfoExportsWithSelectors mi
      (pkgNameFS, _) = modulePackageInfo pkgState flags (Just mdl)
  ...
  let !aliases =
        mkAliasMap pkgState $ tm_renamed_source tm
...
mkAliasMap :: PackageState -> Maybe RenamedSource -> M.Map Module ModuleName
mkAliasMap pkgState mRenamedSource =
...
-- ezyang: Not really...
lookupModuleDyn ::
  PackageState -> Maybe Unit -> ModuleName -> Module
lookupModuleDyn _ (Just pkgId) mdlName =
  Module.mkModule pkgId mdlName
lookupModuleDyn pkgState Nothing mdlName =
  case lookupModuleInAllPackages pkgState mdlName of
    (m,_):_ -> m
