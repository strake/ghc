...
import GHC.Unit.State ( PackageState, lookupUnit, unitPackageIdString )
...
mkModuleTree :: PackageState -> Bool -> [(Module, Maybe (MDoc Name))] -> [ModuleTree]
mkModuleTree pkgState showPkgs mods =
  ...
  where
    ...
    modSrcPkg mod_ | showPkgs = fmap unitPackageIdString
                                     (lookupUnit pkgState (moduleUnit mod_))
                   | otherwise = Nothing
