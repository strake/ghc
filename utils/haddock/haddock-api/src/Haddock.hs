...
render dflags flags sinceQual qual ifaces installedIfaces extSrcMap = do
  ...
  let
    ...
    pkgNameVer       = modulePackageInfo pkgState flags pkgMod
  ...
  when (Flag_GenContents `elem` flags) $ do
    withTiming dflags' "ppHtmlContents" (const ()) $ do
      _ <- {-# SCC ppHtmlContents #-}
           ppHtmlContents pkgState odir title pkgStr
  ...
  when (Flag_Html `elem` flags) $ do
    withTiming dflags' "ppHtml" (const ()) $ do
      _ <- {-# SCC ppHtml #-}
           ppHtml pkgState title pkgStr visibleIfaces reexportedIfaces odir
                  prologue
...
withGhc' libDir needHieFiles flags ghcActs = runGhc (Just libDir) $ do
  ...
  hsc_env <- liftIO . GHC.Runtime.Loader.initializePlugins =<< GHC.getSession
