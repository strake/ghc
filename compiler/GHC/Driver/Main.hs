  ...
  mHscMessage mod_summary source_modified mb_old_iface mod_index
    ...
    let msg what = case mHscMessage of
        ...
        compile mb_old_hash reason = do
            liftIO $ msg reason
            Right . flip (,) mb_old_hash <$> case hscFrontendHook (hsc_hooks hsc_env) of
...
initModDetails hsc_env mod_summary iface =
  fixIO $ \details' -> do
    let f hpt = addToHpt hpt (ms_mod_name mod_summary) (HomeModInfo iface details' Nothing)
        hsc_env' = over hsc_HPTL f hsc_env
...
hscGenHardCode hsc_env cgguts location output_filename = do
        ...
        (stg_binds, (caf_ccs, caf_cc_stacks))
            <- {-# SCC "CoreToStg" #-}
               myCoreToStg dflags (hsc_IC hsc_env) this_mod prepd_binds
        ...
        withTiming dflags
            ...
            rawcmms0 <- {-# SCC "cmmToRawCmm" #-}
                maybe cmmToRawCmm (`flip` Just this_mod) (cmmToRawCmmHook hooks) dflags cmms
...
hscInteractive hsc_env cgguts location = do
    ...
    (stg_binds, _caf_ccs__caf_cc_stacks)
      <- {-# SCC "CoreToStg" #-} myCoreToStg dflags (hsc_IC hsc_env) this_mod prepd_binds
    -----------------  Generate byte code ------------------
...
doCodeGen hsc_env this_mod data_tycons
    ...
    let stg_to_cmm = StgToCmm.codeGen `fromMaybe` stgToCmmHook hooks
...
myCoreToStgExpr :: DynFlags -> InteractiveContext -> Module -> CoreExpr
 -> IO (StgRhs, CollectedCCs)
myCoreToStgExpr dflags ictxt this_mod prepd_expr =
  [ (stg_expr, collected_ccs)
  | -- Create a temporary binding (just because myCoreToStg needs a binding for the stg2stg step)
    let bco_tmp_id = mkSysLocal (fsLit "BCO_toplevel") (mkPseudoUniqueE 0) (exprType prepd_expr)
  , ([StgTopLifted (StgNonRec _ stg_expr)], collected_ccs) <-
       myCoreToStg dflags ictxt this_mod [NonRec bco_tmp_id prepd_expr] ]

myCoreToStg :: DynFlags -> InteractiveContext -> Module -> CoreProgram
            -> IO ( [StgTopBinding] -- output program
                  , CollectedCCs )  -- CAF cost centre info (declared and used)
myCoreToStg dflags ictxt this_mod prepd_binds = do
    ...
    stg_binds2
        <- {-# SCC "Stg2Stg" #-}
           stg2stg dflags ictxt this_mod stg_binds
...
hscCompileCoreExpr hsc_env =
  fromMaybe hscCompileCoreExpr' (hscCompileCoreExprHook (hsc_hooks hsc_env)) hsc_env
