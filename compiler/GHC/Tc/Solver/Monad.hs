...
module GHC.Tc.Solver.Monad (
    ...
    emitWorkNC, emitWork,
    isImprovable,
    inert_cansL, inert_fsksL, inert_flat_cacheL, inert_solved_dictsL,
    inert_eqsL, inert_funeqsL, inert_dictsL, inert_instsL, inert_safehaskL, inert_irredsL,
    ...
    addDictsByClass, delDict, filterDicts, findDict,

    -- Inert CTyEqCans
    EqualCtList, findTyEqs, isInInertEqs,
    lookupInertTyVar,
    ...
    -- References
    newMutVar, readMutVar, writeMutVar, updMutVar,
...
import GHC.Tc.Utils.Monad (newMutVar, readMutVar, updMutVar, writeMutVar)
...
import Data.Functor.Reader.Class (IsLocal (..), IsReader (..))
...
-- Count the things we need to solve excluding the insolubles
...
selectNextWorkItem
  = do { wl_var <- getTcSWorkListRef
       ; wl <- readMutVar wl_var
       ; selectWorkItem wl `for` \ (ct, new_wl) ->
         -- checkReductionDepth (view ctLocL ct) (view ctPredL ct)
         -- This is done by GHC.Tc.Solver.Interact.chooseInstance
         ct <$ writeMutVar wl_var new_wl }
...
instance Outputable WorkList where
  ppr (WL { wl_eqs = eqs, wl_funeqs = feqs
          , wl_rest = rest, wl_implics = implics })
   = text "WL" <+> (braces $
     vcat [ munless (null eqs) $
            text "Eqs =" <+> vcat (map ppr eqs)
          , munless (null feqs) $
            text "Funeqs =" <+> vcat (map ppr feqs)
          , munless (null rest) $
            text "Non-eqs =" <+> vcat (map ppr rest)
          , munless (isEmptyBag implics) $
...
LENS_FIELD(inert_cansL, inert_cans)
LENS_FIELD(inert_fsksL, inert_fsks)
LENS_FIELD(inert_flat_cacheL, inert_flat_cache)
LENS_FIELD(inert_solved_dictsL, inert_solved_dicts)
...
instance Outputable InertSet where
  ppr (IS { inert_cans = ics
      ...
      = vcat [ ppr ics
         ...
         where
           dicts = toList (toBag solved_dicts)

emptyInertCans :: InertCans
emptyInertCans
  = IC { inert_eqs      = mapEmpty
...
LENS_FIELD(inert_eqsL, inert_eqs)
LENS_FIELD(inert_funeqsL, inert_funeqs)
LENS_FIELD(inert_dictsL, inert_dicts)
LENS_FIELD(inert_instsL, inert_insts)
LENS_FIELD(inert_safehaskL, inert_safehask)
LENS_FIELD(inert_irredsL, inert_irreds)
...
instance Outputable InertCans where
  ppr (IC { inert_eqs = eqs
    ...
    = braces $ vcat
      [ munless (null eqs) $
        text "Equalities:"
          <+> pprCts (foldr (andCts . toBag) emptyCts eqs)
      , munless (null funeqs) $
        text "Type-function equalities =" <+> pprCts (toBag (Compose funeqs))
      , munless (null dicts) $
        text "Dictionaries =" <+> pprCts (toBag (Compose dicts))
      , munless (null safehask) $
        text "Safe Haskell unsafe overlap =" <+> pprCts (toBag (Compose safehask))
      , munless (isEmptyCts irreds) $
...
addTyEq :: TcTyVar -> Ct -> InertEqs -> InertEqs
addTyEq tv ct old_eqs = extendDVarEnv_C add_eq old_eqs tv [ct]
  where
    add_eq old_eqs _
      | isWantedCt ct
      , (eq1 : eqs) <- old_eqs
...
delTyEq :: TcTyVar -> TcType -> InertEqs -> InertEqs
delTyEq tv t = mapAdjust (filter (not . isThisOne)) tv
...
addInertForAll new_qci
  ...
  where
    ...
    same_qci old_qci = (tcEqType `on` ctEvPred . qci_ev) old_qci new_qci
...
addInertCan ct
  = do { traceTcS "insertInertCan {" $
       ...
       ; ics <- maybeKickOut ct ics
       ; setInertCans (add_item ct ics)

       ; traceTcS "addInertCan }" mempty }

maybeKickOut :: Ct -> InertCans -> TcS InertCans
-- For a CTyEqCan, kick out any inert that can be rewritten by the CTyEqCan
maybeKickOut ct
  | CTyEqCan { cc_tyvar = tv, cc_ev = ev, cc_eq_rel = eq_rel } <- ct
  = fmap snd . kickOutRewritable (ctEvFlavour ev, eq_rel) tv
  | otherwise
  = pure

add_item :: Ct -> InertCans -> InertCans
add_item item@CFunEqCan { cc_fun = tc, cc_tyargs = tys } = over inert_funeqsL $ insertFunEq tc tys item
add_item item@CTyEqCan { cc_tyvar = tv } = over inert_eqsL $ addTyEq tv item
add_item item@CIrredCan {} = over inert_irredsL (`Bag.snocBag` item)
add_item item@CDictCan { cc_class = cls, cc_tyargs = tys } = over inert_dictsL $ addDict cls tys item
add_item item
...
kick_out_rewritable new_fr new_tv
  ...
  where
    ...
    (irs_out,    irs_in)    = partition        kick_out_ct irreds
    ...
    (insts_out, insts_in)
       ...
       = mapEither kick_out_qci old_insts
       | otherwise
       = ([], old_insts)
    kick_out_qci qci
      | let ev = qci_ev qci
...
kickOutAfterFillingCoercionHole hole
  ...
  where
    ...
    kick_out ics@(IC { inert_irreds = irreds })
      = let (to_kick, to_keep) = partition kick_ct irreds

            kicked_out = extendWorkListCts (toList to_kick) emptyWorkList
...
--------------
addInertSafehask :: Ct -> InertCans -> InertCans
addInertSafehask item@CDictCan { cc_class = cls, cc_tyargs = tys }
  = over inert_safehaskL $ addDict cls tys item

addInertSafehask item
...
insertSafeOverlapFailureTcS what item
  ...
  | otherwise        = updInertCans (addInertSafehask item)

getSafeOverlapFailures :: TcS Cts
-- See Note [Safe Haskell Overlapping Instances Implementation] in GHC.Tc.Solver
getSafeOverlapFailures = getInertCans <₪> \ IC { inert_safehask = safehask } ->
   foldr consCts emptyCts (Compose safehask)
...
addSolvedDict what item cls tys
  ...
  = do { traceTcS "updSolvedSetTcs:" $ ppr item
       ; updInertTcS $ over inert_solved_dictsL $ addDict cls tys item }
  | otherwise
  = return ()

getSolvedDicts :: TcS (DictMap CtEvidence)
getSolvedDicts = inert_solved_dicts <$> getTcSInerts

setSolvedDicts :: DictMap CtEvidence -> TcS ()
setSolvedDicts = updInertTcS . set inert_solved_dictsL
...
updInertTcS fn = getTcSInertsRef >>= wrapTcS . flip TcM.updMutVar fn

getInertCans :: TcS InertCans
getInertCans = inert_cans <$> getTcSInerts

setInertCans :: InertCans -> TcS ()
setInertCans = updInertTcS . set inert_cansL

updRetInertCans :: (InertCans -> (a, InertCans)) -> TcS a
-- Modify the inert set with the supplied function
updRetInertCans upd_fn
  = do { is_var <- getTcSInertsRef
       ; wrapTcS (do { inerts <- TcM.readMutVar is_var
                     ; let (res, cans') = upd_fn (inert_cans inerts)
                     ; res <$ TcM.writeMutVar is_var (inerts { inert_cans = cans' }) }) }

updInertCans :: (InertCans -> InertCans) -> TcS ()
-- Modify the inert set with the supplied function
updInertCans = updInertTcS . over inert_cansL

updInertDicts :: (DictMap Ct -> DictMap Ct) -> TcS ()
-- Modify the inert set with the supplied function
updInertDicts = updInertTcS . over (inert_cansL . inert_dictsL)

updInertSafehask :: (DictMap Ct -> DictMap Ct) -> TcS ()
-- Modify the inert set with the supplied function
updInertSafehask = updInertTcS . over (inert_cansL . inert_safehaskL)

updInertFunEqs :: (FunEqMap Ct -> FunEqMap Ct) -> TcS ()
-- Modify the inert set with the supplied function
updInertFunEqs = updInertTcS . over (inert_cansL . inert_funeqsL)

updInertIrreds :: (Cts -> Cts) -> TcS ()
-- Modify the inert set with the supplied function
updInertIrreds = updInertTcS . over (inert_cansL . inert_irredsL)

getInertEqs :: TcS (DTyVarEnv EqualCtList)
getInertEqs = inert_eqs <$> getInertCans

getInertInsols :: TcS Cts
-- Returns insoluble equality constraints
-- specifically including Givens
getInertInsols = filter insolubleEqCt . inert_irreds <$> getInertCans

getInertGivens :: TcS [Ct]
-- Returns the Given constraints in the inert set,
-- with type functions *not* unflattened
getInertGivens =
  [ filter isGivenCt all_cts
  | inerts <- getInertCans
  , let all_cts = foldr (:) `flip` Compose (inert_dicts inerts)
                $ foldr (:) `flip` Compose (inert_funeqs inerts)
                $ concat (toList (inert_eqs inerts)) ]
...
get_sc_pending this_lvl ic@(IC { inert_dicts = dicts, inert_insts = insts })
  ...
  where
    ...
    sc_pend_dicts = foldr get_pending [] (Compose dicts)
    ...
    add ct@CDictCan { cc_class = cls, cc_tyargs = tys } = addDict cls tys ct
    add ct = pprPanic "getPendingScDicts" (ppr ct)
...
getUnsolvedInerts
 = do { IC { inert_eqs    = tv_eqs
      ...
      ; let unsolved_tv_eqs  = foldr add_if_unsolved emptyCts (Compose tv_eqs)
...
getNoGivenEqs tclvl skol_tvs
  = do { inerts@(IC { inert_eqs = ieqs, inert_irreds = irreds })
       ...
       ; let has_given_eqs = foldr ((||) . ct_given_here) False irreds
                          || any eqs_given_here ieqs
             insols = filter insolubleEqCt irreds
...
matchableGivens loc_w pred_w (IS { inert_cans = inert_cans })
  = filter matchable_given all_relevant_givens
  where
...
removeInertCt is ct =
  case ct of
    ...
    CDictCan  { cc_class = cl, cc_tyargs = tys } -> over inert_dictsL (delDict cl tys) is

    CFunEqCan { cc_fun  = tf,  cc_tyargs = tys } -> over inert_funeqsL (delFunEq tf tys) is

    CTyEqCan  { cc_tyvar = x,  cc_rhs    = ty } -> over inert_eqsL (delTyEq x ty) is
...
lookupInertDict (IC { inert_dicts = dicts }) loc cls tys
  = ctEvidence <$> findDict dicts loc cls tys
...
delTcApp :: Unique -> [Type] -> TcAppMap a -> TcAppMap a
delTcApp cls tys m = adjustUDFM (mapDelete tys) m cls

insertTcApp :: Unique -> [Type] -> a -> TcAppMap a -> TcAppMap a
insertTcApp cls tys ct = mapAlter alter_tm cls
  where
...
delDict :: Class -> [Type] -> DictMap a -> DictMap a
delDict = delTcApp . getUnique

addDict :: Class -> [Type] -> a -> DictMap a -> DictMap a
addDict = insertTcApp . getUnique

addDictsByClass :: DictMap Ct -> Class -> Bag Ct -> DictMap Ct
addDictsByClass m cls items = addToUDFM_Directly m (getUnique cls) (foldr add mapEmpty items)
  where
    add ct@CDictCan { cc_tyargs = tys } = mapInsert tys ct
    add ct = pprPanic "addDictsByClass" (ppr ct)
...
partitionDicts f = foldr k (empty, emptyDicts) . Compose
  where
    ...
    add ct@CDictCan { cc_class = cls, cc_tyargs = tys } = addDict cls tys ct
    add ct = pprPanic "partitionDicts" (ppr ct)
...
findFunEq m tc = findTcApp m (getUnique tc)
...
insertFunEq :: TyCon -> [Type] -> a -> FunEqMap a -> FunEqMap a
insertFunEq = insertTcApp . getUnique
...
partitionFunEqs f m = (yeses, foldr del m yeses)
  where
    yeses = foldr k [] (Compose m)
    k ct | f ct = (ct :) | otherwise = id
    del CFunEqCan { cc_fun = tc, cc_tyargs = tys } = delFunEq tc tys
    del ct = pprPanic "partitionFunEqs" (ppr ct)

delFunEq :: TyCon -> [Type] -> FunEqMap a -> FunEqMap a
delFunEq = delTcApp . getUnique
...
LENS_FIELD(tcs_ev_bindsL, tcs_ev_binds)
LENS_FIELD(tcs_unifiedL, tcs_unified)
LENS_FIELD(tcs_countL, tcs_count)
LENS_FIELD(tcs_inertsL, tcs_inerts)
LENS_FIELD(tcs_worklistL, tcs_worklist)
...
instance IsLocal TcS TcS where
    local f (TcS a) = TcS (a . f)
...
bumpStepCountTcS = TcS $ \env -> TcM.updMutVar (tcs_count env) (+1)
...
traceFireTcS ev doc
  = TcS $ \env -> csTraceTcM $
  [ hang (text "Step" <+> int n
                       <> brackets (text "l:" <> ppr tclvl <> comma <>
                                    text "d:" <> ppr (ctLocDepth (ctEvLoc ev)))
                       <+> doc <> colon)
                     4 (ppr ev)
  | n <- TcM.readMutVar (tcs_count env), tclvl <- TcM.getTcLevel ]
...
csTraceTcM mk_doc
  = do { dflags <- getDynFlags
       ; when (dopt Opt_D_dump_cs_trace dflags || dopt Opt_D_dump_tc_trace dflags) $
         do { msg <- mk_doc
            ; TcM.dumpTcRn False (dumpOptionsFromFlag Opt_D_dump_cs_trace) "" FormatText msg } }
...
runTcS tcs
  = do { ev_binds_var <- TcM.newTcEvBinds
       ; (,) <$> runTcSWithEvBinds ev_binds_var True tcs <*> TcM.getTcEvBindsMap ev_binds_var }
...
runTcSWithEvBinds ev_binds_var unflatten tcs
  = do { unified_var <- TcM.newMutVar 0
       ; step_count <- TcM.newMutVar 0
       ; inert_var <- TcM.newMutVar emptyInert
       ; wl_var <- TcM.newMutVar emptyWorkList
       ...
       ; count <- TcM.readMutVar step_count
       ; when (count > 0) $
...
checkForCyclicBinds ev_binds_map
  ...
  where
    ...
    edges = [ DigraphNode bind bndr (nonDetEltsUniqSet (evVarsOfTerm rhs))
            | bind@(EvBind { eb_lhs = bndr, eb_rhs = rhs}) <- toList ev_binds ]
...
setEvBindsTcS ref = local (set tcs_ev_bindsL ref)
...
nestImplicTcS ref inner_tclvl (TcS thing_inside)
  = TcS $ \ TcSEnv { tcs_unified       = unified_var
    ...
    do { inerts <- TcM.readMutVar old_inert_var
       ; let nest_inert = emptyInert
                            { inert_cans = inert_cans inerts
                            , inert_solved_dicts = inert_solved_dicts inerts }
                              -- See Note [Do not inherit the flat cache]
       ; new_inert_var <- TcM.newMutVar nest_inert
       ; new_wl_var    <- TcM.newMutVar emptyWorkList
...
nestTcS (TcS thing_inside)
  = TcS $ \ env@(TcSEnv { tcs_inerts = inerts_var }) ->
    do { inerts <- TcM.readMutVar inerts_var
       ; new_inert_var <- TcM.newMutVar inerts
       ; new_wl_var    <- TcM.newMutVar emptyWorkList
       ...
       ; new_inerts <- TcM.readMutVar new_inert_var
       ...
       ; res <$ TcM.writeMutVar inerts_var  -- See Note [Propagate the solved dictionaries]
...
emitImplicationTcS new_tclvl skol_info skol_tvs givens wanteds
  = do { let wc = emptyWC { wc_simple = wanteds }
       ...
       ; TcEvBinds (ic_binds imp) <$ emitImplication imp }
...
getTcSInerts = getTcSInertsRef >>= readMutVar

setTcSInerts :: InertSet -> TcS ()
setTcSInerts ics = do { r <- getTcSInertsRef; writeMutVar r ics }

getWorkListImplics :: TcS (Bag Implication)
getWorkListImplics
  = do { wl_var <- getTcSWorkListRef
       ; wl_implics <$> readMutVar wl_var }

pushLevelNoWorkList :: SDoc -> TcS a -> TcS (TcLevel, a)
-- Push the level and run thing_inside
-- However, thing_inside should not generate any work items
#if defined(DEBUG)
pushLevelNoWorkList err_doc (TcS thing_inside) = TcS \env -> TcM.pushTcLevelM $
    thing_inside env { tcs_worklist = wl_panic }
  where
    wl_panic  = pprPanic "GHC.Tc.Solver.Monad.buildImplication" err_doc
    -- This panic checks that the thing-inside does not emit any work-list constraints
#else
pushLevelNoWorkList _ (TcS thing_inside) = TcS \env -> TcM.pushTcLevelM $
    thing_inside env  -- Don't check
#endif
...
emitWorkNC evs = null evs `unless` emitWork (map mkNonCanonical evs)
...
emitImplication implic = updWorkListTcS (extendWorkListImplic implic)
...
getTcEvTyCoVars ev_binds_var = wrapTcS $ TcM.getTcEvTyCoVars ev_binds_var

getTcEvBindsMap :: EvBindsVar -> TcS EvBindMap
getTcEvBindsMap ev_binds_var = wrapTcS $ TcM.getTcEvBindsMap ev_binds_var

setTcEvBindsMap :: EvBindsVar -> EvBindMap -> TcS ()
setTcEvBindsMap ev_binds_var binds = wrapTcS $ TcM.setTcEvBindsMap ev_binds_var binds
...
unifyTyVar tv ty
  = assertPpr (isMetaTyVar tv) (ppr tv) $
    ...
    do { TcM.traceTc "unifyTyVar" (ppr tv <+> text ":=" <+> ppr ty)
       ...
       ; TcM.updMutVar (tcs_unified env) (+1) }

reportUnifications :: TcS a -> TcS (Int, a)
reportUnifications (TcS thing_inside)
  = TcS $ \ env ->
    do { inner_unified <- TcM.newMutVar 0
       ; res <- thing_inside (env { tcs_unified = inner_unified })
       ; n_unifs <- TcM.readMutVar inner_unified
       ; (n_unifs, res) <$ TcM.updMutVar (tcs_unified env) (+ n_unifs) }
...
newFlattenSkolem flav loc tc xis
  = do { stuff@(ev, co, fsk) <- new_skolem
       ...
       ; stuff <$ extendFlatCache tc xis (co, fsk_ty, ctEvFlavour ev) }
  where
    ...
    new_skolem
      ...
      = do { fmv <- wrapTcS (TcM.newFmvTyVar fam_ty)
           ...
           ; newWantedEq_SI NoBlockSubst WDeriv loc Nominal fam_ty (mkTyVarTy fmv) <₪>
             \ (ev, co) -> (ev, co, fmv) }
...
unflattenGivens inert_var
 = do { inerts <- TcM.readMutVar inert_var
       ; TcM.traceTc "unflattenGivens" (ppr (inert_fsks inerts))
       ; traverse_ flatten_one (inert_fsks inerts) }
  where
...
extendFlatCache tc xi_args stuff@(_, ty, fl)
  ...
  = do { dflags <- getDynFlags
    ...
    do { traceTcS "extendFlatCache" (vcat [ ppr tc <+> ppr xi_args
       ...
       ; updInertTcS $ over inert_flat_cacheL $ \ fc -> insertExactFunEq fc tc xi_args stuff } }
...
instDFunType dfun_id inst_tys = wrapTcS $ TcM.instDFunType dfun_id inst_tys
...
instFlexiX subst tvs = wrapTcS (foldlM instFlexiHelper subst tvs)

instFlexiHelper :: TCvSubst -> TKVar -> TcM TCvSubst
instFlexiHelper subst tv =
  [ extendTvSubst subst tv ty'
  | uniq <- TcM.newUnique
...
useVars co_vars
  = do { ev_binds_var <- getTcEvBindsVar
       ...
       ; wrapTcS $
         do { tcvs <- TcM.readMutVar ref
            ; let tcvs' = tcvs `unionVarSet` co_vars
            ; TcM.writeMutVar ref tcvs' } }
...
setEvBindIfWanted ev tm = case ev of
...
newGivenEvVar loc (pred, rhs) =
  [ CtGiven { ctev_pred = pred, ctev_evar = new_ev, ctev_loc = loc }
  | new_ev <- newBoundEvVarId pred rhs ]
...
newBoundEvVarId pred rhs
  = do { new_ev <- newEvVar pred
       ; new_ev <$ setEvBind (mkGivenEvBind new_ev rhs) }

newGivenEvVars :: CtLoc -> [(TcPredType, EvTerm)] -> TcS [CtEvidence]
newGivenEvVars = traverse . newGivenEvVar

emitNewWantedEq :: CtLoc -> Role -> TcType -> TcType -> TcS Coercion
-- | Emit a new Wanted equality into the work-list
emitNewWantedEq loc role ty1 ty2
  = do { (ev, co) <- newWantedEq loc role ty1 ty2
       ; co <$ updWorkListTcS (extendWorkListEq (mkNonCanonical ev)) }
...
newWantedEq_SI blocker si loc role ty1 ty2 =
  [ ( CtWanted { ctev_pred = pty, ctev_dest = HoleDest hole, ctev_nosh = si, ctev_loc = loc}
    , mkHoleCo hole )
  | hole <- wrapTcS $ TcM.newCoercionHole blocker pty
...
newWantedEvVarNC_SI si loc pty =
  [ CtWanted { ctev_pred = pty, ctev_dest = EvVarDest new_ev, ctev_nosh = si, ctev_loc = loc }
  | new_ev <- newEvVar pty
  , () <- traceTcS "Emitting new wanted" (ppr new_ev <+> dcolon <+> ppr pty $$ pprCtLoc loc) ]
...
newWantedEvVar_SI si loc pty = lookupInInerts loc pty >>= \ case
            Just ctev | not (isDerived ctev)
              -> Cached (ctEvExpr ctev) <$ traceTcS "newWantedEvVar/cache hit" (ppr ctev)
            _ -> Fresh <$> newWantedEvVarNC_SI si loc pty
...
emitNewDeriveds loc preds = null preds `unless`
    do { evs <- traverse (newDerivedNC loc) preds
...
newDerivedNC loc pred
  = -- do checkReductionDepth loc pred;
    pure CtDerived { ctev_pred = pred, ctev_loc = loc }
...
matchFamTcM tycon args =
  ...
  , let match_fam_result = reduceTyFamApp_maybe fam_envs Nominal tycon args
