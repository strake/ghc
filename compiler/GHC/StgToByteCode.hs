...
stgExprToBCOs hsc_env this_mod expr_ty expr
 = withTiming dflags
  ...
  where dflags = hsc_dflags hsc_env
        ...
        dummy_id = mkSysLocal (fsLit "BCO_toplevel") (mkPseudoUniqueE 0) expr_ty
{-
...
fvsToEnv p (StgRhsClosure fvs _ _ _ _) = [v | v <- toList fvs, Map.member v p]
fvsToEnv _ _ = []
...
schemeE d s p (StgLetNoEscape xlet bnd body) = schemeE d s p (StgLet xlet bnd body)
...
schemeE _d _s _p (StgTick (Breakpoint _ bp_id _) _rhs)
   = panic ("schemeE: Breakpoint without let binding: " ++
            show bp_id ++ " forgot to run bcPrep?")
...
schemeE d s p (StgCase scrut bndr _ alts) = doCase d s p scrut bndr alts
...
doCase d s p scrut bndr alts
  = do
     ...
     bool
         ...
         [ PUSH_ALTS_TUPLE alt_bco' tuple_info tuple_bco `consOL` scrut_code
         | let args_ptrs = args_offsets <₪> \ (rep, off) ->
                   (isFollowableArg (toArgRep rep), off)
         , tuple_bco <- emitBc (tupleBCO platform tuple_info args_ptrs) ]
           ubx_tuple_frame
...
idSizeCon platform var
  ...
  | isUnboxedTupleType (idType var) ||
    ...
    WordOff . sum . map (argRepSizeW platform . toArgRep) .
    bcIdPrimReps $ var
...
typeArgReps = fmap toArgRep . typePrimRepArgs
...
getCCArray = modBreaks_ccs . expectJust "GHC.StgToByteCode.getCCArray" . modBreaks <$> get
