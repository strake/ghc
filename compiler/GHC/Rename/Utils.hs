...
module GHC.Rename.Utils (
        ...
        bindLocalNames, bindLocalNamesFV, bindLocalNamesFVW,
...
newLocalBndrRn (L loc rdr_name)
  ...
  | otherwise
  = do { isUnqual rdr_name `unless` addErrAt loc (badQualBndrErr rdr_name)
...
bindLocalNamesFVW :: [Name] -> WriterT FreeVars RnM a -> WriterT FreeVars RnM a
bindLocalNamesFVW names = censor (delFVs names) . bindLocalNames names

-------------------------------------

checkDupRdrNames :: [Located RdrName] -> RnM ()
-- Check for duplicated names in a binding group
checkDupRdrNames rdr_names_w_loc = traverse_ (dupNamesErr getLoc) dups
  where
...
check_dup_names names = traverse_ (dupNamesErr nameSrcSpan) dups
  where
...
warnUnusedTopBinds gres
    ...
    $ do env <- getGblEnv
         ...
         let noParent gre = case gre_par gre of
             ...
             gres' = bool id (filter noParent) isBoot gres
         traverse_ warnUnusedGRE gres'
...
warnUnusedGRE gre@GRE { gre_name = name, gre_lcl = lcl, gre_imp = is }
  ...
  | otherwise = when (reportable name occ) (traverse_ warn is)
  where
    occ = greOccName gre
